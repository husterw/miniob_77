...
{
public:
  InsertLogicalOperator(Table *table, vector<vector<Value>> values);
  ...
  const vector<vector<Value>> &values() const { return values_; }
  vector<vector<Value>>       &values() { return values_; }
