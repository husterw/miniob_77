...
RC HeapTableEngine::create_index(Trx *trx, const vector<const FieldMeta *> &field_metas, const char *index_name, bool Unique)
{
  if (common::is_blank(index_name) || field_metas.empty()) {
    LOG_INFO("Invalid input arguments, table name is %s, index_name is blank or no fields specified", table_meta_->name());
    return RC::INVALID_ARGUMENT;
  }
  ...
  for (const auto *field_meta : field_metas) {
    if (field_meta == nullptr) {
      LOG_INFO("Invalid field_meta in field_metas");
      return RC::INVALID_ARGUMENT;
    }
    fields.push_back(*field_meta);
  }

  RC rc = new_index_meta.init(index_name, fields, Unique);
  if (rc != RC::SUCCESS) {
